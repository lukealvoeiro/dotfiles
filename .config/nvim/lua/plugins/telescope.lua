return {
  ...
  keys = function()
    ...
    return {
      ...
      { "<leader>sg", function() fzf.live_grep({ rg_opts = "--hidden --column --line-number --no-heading --color=always --smart-case --max-columns=4096 --fixed-strings" }) end, desc = "[S]earch by [G]rep (literal)" },
      { "<leader>sG", fzf.live_grep, desc = "[S]earch by [G]rep (regex)" },
