...
alias gg="lazygit"
alias cat="bat -p"
alias droid="/Users/luke/.local/bin/droid"
...
# Rewrite `git push -f` / `git push --force` to `git push --force-with-lease`
git() {
  if [[ "$1" == "push" ]]; then
    local args=("${@:2}")
    local new_args=()
    for arg in "${args[@]}"; do
      case "$arg" in
        -f|--force) new_args+=(--force-with-lease) ;;
        *) new_args+=("$arg") ;;
      esac
    done
    command git push "${new_args[@]}"
  else
    command git "$@"
  fi
}
